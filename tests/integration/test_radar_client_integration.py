...
class TestRadarClientIntegration:
    ...
    def test_search_places_real(self, radar_client: RadarClient):
        ...
        result = radar_client.search_places(
            near="40.7342,-73.9912", chains="starbucks", radius=5000
        )
        ...
        assert any(
            "coffee" in cat.lower() or "cafe" in cat.lower() for cat in place.categories
        )
    ...
    def test_autocomplete_real(self, radar_client: RadarClient):
        ...
        assert len(result.addresses) > 0
        found = any(
            "Broadway" in (addr.formattedAddress or "") for addr in result.addresses
        )
        assert found
